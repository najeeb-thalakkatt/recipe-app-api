...
class PrivateRecipeAPITests(TestCase):
    """Test all authenticated API requests"""
    ...
    def test_retrive_limited_to_user(self):
    ...
    def test_update_user_returns_error(self):
        """Test chnaging  the recipe user result in error"""
    ...
    def test_other_user_recipe_error(self):
        ...
        new_user = create_user(email='user2@emaple.com', password='pass123')
        recipe = create_recipe(
            user=new_user
        )
        url = detail_url(recipe.id)
    ...
    def test_create_recipe_with_new_ingredients(self):
        ...
        payload = {
            ...
            'time_minutes': 30,
            'price': Decimal('2.50'),
            'ingredients': [{'name': 'prawn'}, {'name': 'salt'}]
        }
        res = self.client.post(RECIPE_URL, payload, format='json')
    ...
    def test_create_ingredient_on_update(self):
        ...
        payload = {'ingredients': [{'name': 'peppar'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        peppar = Ingredient.objects.get(user=self.user, name='peppar')
        self.assertIn(peppar, recipe.ingredients.all())
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        peppar = Ingredient.objects.create(user=self.user, name='peppar')
    ...
    def test_filter_by_tags(self):
        ...
        r2 = create_recipe(user=self.user, title='Ramen')
    ...
    def test_filter_by_ingredientss(self):
        """Tests filtering recipes by tags."""
        r1 = create_recipe(user=self.user, title='Thai veg curry')
        r2 = create_recipe(user=self.user, title='Ramen')
        r3 = create_recipe(user=self.user, title='Fish and chips')
        in1 = Ingredient.objects.create(user=self.user, name='Rice')
        in2 = Ingredient.objects.create(user=self.user, name='Noodles')
        in3 = Ingredient.objects.create(user=self.user, name='potato')

        r1.ingredients.add(in1)
        r2.ingredients.add(in2)
        r3.ingredients.add(in3)
